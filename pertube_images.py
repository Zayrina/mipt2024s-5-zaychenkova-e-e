'''
GuoWang xie https://github.com/gwxie/Synthesize-Distorted-Image-and-Its-Control-Points
set up :2020-1-9
...
class perturbed(pertube_utils.BasePerturbed):
	...
	def save_img(self, m, n, fold_curve='fold', repeat_time=4, fiducial_points = 31, relativeShift_position='relativeShift_v2'):
		...
		perfix_ = self.save_suffix + '_' + str(m)
		is_shrink = False
		...
		cv2.imwrite(self.save_path + 'fiducial_points/' + perfix_ + '_' + str(Path(perturbed_bg_img_).stem) + '_' + fold_curve + '.png', fiducial_points_synthesis_perturbed_img)

		cv2.imwrite(self.save_path + 'png/' + perfix_ + '_' + str(Path(perturbed_bg_img_).stem) + '_' + fold_curve + '.png', self.synthesis_perturbed_color[:, :, :3])
		...
		cv2.imwrite(self.save_path + 'outputs/' + perfix_ + '_' + str(Path(perturbed_bg_img_).stem) + '_' + fold_curve + '.png', flat_img)
		'''forward-end'''
		
		'''or'''
		print(f"fiducial_points_coordinate: {fiducial_points_coordinate.shape}")
		...
		with open(self.save_path+'color/'+perfix_+'_'+ str(Path(perturbed_bg_img_).stem) + '_'+fold_curve+'.gw', 'wb') as f:
...
def xgw(args):
	...
	save_suffix = args.path

	all_img_path = getDatasets(path)
	
	global begin_train
	begin_train = time.time()
	fiducial_points = 61	# 61 31
	# process_pool = Pool(5)
	for m, img_path in tqdm(enumerate(all_img_path)):
		# for n in range(args.sys_num):
			n = 0    
			img_path_ = f"{path}/{img_path}"
			bg_path_ = bg_path
			...
			repeat_time = 1     
			saveFold.save_img(img_path, n, 'fold', repeat_time, fiducial_points, 'relativeShift_v2')
			saveCurve.save_img(img_path, n, 'curve', repeat_time, fiducial_points, 'relativeShift_v2')
...
if __name__ == '__main__':
  args = Args()
  args.output_path = 'pertubed_barcodes_7/'
  args.bg_path = 'background/'
  args.path = 'transformed_data'
  args.sys_num = 1
  args.repeat_T = 0
  xgw(args)